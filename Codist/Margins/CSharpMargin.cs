    ...
    sealed class MemberMarker {
      ...
      sealed class PenStore {
        ...
        internal Pen GetPenForCodeMemberType(CodeMemberType memberType) {
          switch (memberType) {
            case CodeMemberType.Class:
              return Class;
            case CodeMemberType.Interface:
              return Interface;
            case CodeMemberType.Struct:
              return Struct;
            case CodeMemberType.Enum:
              return Enum;
            case CodeMemberType.Event:
              return Event;
            case CodeMemberType.Delegate:
              return Delegate;
            case CodeMemberType.Constructor:
              return Constructor;
            case CodeMemberType.Property:
              return Property;
            case CodeMemberType.Method:
              return Method;
            case CodeMemberType.Field:
              return Field;
          }
          return null;
        }
    ...
    sealed class SymbolReferenceMarker {
      ...
      sealed class PenStore {
        ...
        public PenStore() {
          ...
          WriteMarker = (SolidColorBrush)new SolidColorBrush(config.WriteMarker).MakeFrozen();
          ReferenceMarker = (SolidColorBrush)new SolidColorBrush(config.ReferenceMarker).MakeFrozen();
