...
static class FormatStore
{
	...
	static bool IsFormattable(IClassificationType item) {
		...
		return (item != null)
			...
			&& !ServicesHelper.Instance.ClassificationTypeExporter.ShouldIgnore(item);
	}
	...
	sealed class Highlighter(string category, IEditorFormatMap editorFormatMap, IClassificationFormatMap classificationFormatMap) : IEquatable<Highlighter>, IFormatCache
	{
		const string UnnecessaryCodeDiagnostic = "UnnecessaryCodeDiagnostic";
		readonly string _Category = category;
		readonly IEditorFormatMap _EditorFormatMap = editorFormatMap;
		readonly IClassificationFormatMap _ClassificationFormatMap = classificationFormatMap;
		// traces changed IEditorFormatMap items;
		// key should obtain from IClassificationFormatMap.GetEditorFormatMapKey
		readonly Dictionary<string, ChangeTrace> _Traces = [];
		readonly Dictionary<IClassificationType, TextFormattingRunProperties> _PropertiesCache = new(new ClassificationTypeComparer());
		readonly Dictionary<string, IClassificationType> _FormatClassificationTypes = [];
		readonly Stack<string> _Formatters = new();
		readonly List<string> _ChangedFormatItems = new(7);
		readonly PendingChange _PendingChange = new();
		FormatContext _Context;
		Color _ViewBackground = editorFormatMap.GetBackgroundColor();
		Typeface _DefaultTypeface = classificationFormatMap.DefaultTextProperties.Typeface;
		double _DefaultFontSize = classificationFormatMap.DefaultTextProperties.FontRenderingEmSize;
		int _Lock;
		...
		void ClassificationFormatMappingChanged(object sender, EventArgs e) {
			...
			try {
				...
				if (!StyleBase.AreEqual(currentTypeface, _DefaultTypeface)) {
		...
		/// </summary>
		sealed class ChangeTrace(ResourceDictionary origin)
		{
			...
			/// </summary>
			public readonly ResourceDictionary Origin = origin.Copy();
			...
			void ChangeBrush(ResourceDictionary current, StyleBase style, Highlighter highlighter, ref FormatChanges c) {
				...
				if (style != null && style.ForeColor.A != 0) {
					if (!StyleBase.AreEqual(b = style.MakeBrush(highlighter._InvertBrightness), current.GetBrush())) {
				...
				else if (!StyleBase.AreEqual(b = current.GetBrush(), Origin.GetBrush())) {
					Origin.SetBrush(b);
					c |= FormatChanges.ForegroundBrush;
				}
			}
			...
			void ChangeBackgroundBrush(ResourceDictionary current, StyleBase style, Highlighter highlighter, ref FormatChanges c) {
				...
				if (style != null && style.BackColor.A != 0) {
					...
					if (!StyleBase.AreEqual(b, current.GetBackgroundBrush())) {
				...
				else if (!StyleBase.AreEqual(b = current.GetBackgroundBrush(), Origin.GetBackgroundBrush())) {
			...
			void ChangeTypeface(ResourceDictionary current, StyleBase style, Highlighter highlighter, ref FormatChanges c) {
				...
				if (style != null && String.IsNullOrEmpty(style.Font) == false) {
					...
					if (ct != null
						&& !StyleBase.AreEqual(current.GetTypeface(), ct)
						&& !StyleBase.AreEqual(ct, highlighter._DefaultTypeface)) {
				...
				else if (!StyleBase.AreEqual(ct = current.GetTypeface(), Origin.GetTypeface())) {
					Origin.SetTypeface(ct);
					c |= FormatChanges.Typeface;
				}
			}

			// note: The equality comparison of TextDecorationCollection not correctly implemented,
			//   consequently t?.SequenceEqual(td) != true may return true
			//   even when contents in the collections are actually the same
			void ChangeTextDecorations(ResourceDictionary current, StyleBase style, Highlighter highlighter, ref FormatChanges c) {
				...
				if (style?.HasLineStyle == true) {
					...
					if (!StyleBase.AreEqual(t, td = current.GetTextDecorations())) {
				...
				else if (!StyleBase.AreEqual(td = current.GetTextDecorations(), Origin.GetTextDecorations())) {
