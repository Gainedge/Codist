...
abstract class StyleBase
{
	...
	/// <summary>
	/// Compares brushes made by this class.
	/// </summary>
	internal static bool AreEqual(Brush x, Brush y) {
		if (x == y) {
			return true;
		}
		if (x == null || y == null) {
		...
		if (x is LinearGradientBrush gx && y is LinearGradientBrush gy) {
			return gx.EndPoint == gy.EndPoint
				&& gx.GradientStops.Count == 2
				&& gx.GradientStops.Count == gy.GradientStops.Count
				&& AreEqual(gx.GradientStops[0], gy.GradientStops[0])
				&& AreEqual(gx.GradientStops[1], gy.GradientStops[1]);
		}
		return x.Equals(y);
	}

	static bool AreEqual(GradientStop x, GradientStop y) {
		return x.Offset == y.Offset && x.Color == y.Color;
	}
	...
	internal static bool AreEqual(Typeface x, Typeface y) {
		if (x == y) {
			return true;
		}
	...
	/// <summary>
	/// Compares TextDecorationCollection made by this class.
	/// </summary>
	internal static bool AreEqual(TextDecorationCollection x, TextDecorationCollection y) {
		if (x == y) {
			return true;
		}
		int count;
		if (x is null || y is null || (count = x.Count) != y.Count) {
			return false;
		}
	...
	static bool AreEqual(TextDecoration x, TextDecoration y) {
		return x.Location == y.Location
			&& x.PenOffset == y.PenOffset
			&& AreEqual(x.Pen, y.Pen)
			&& x.PenThicknessUnit == y.PenThicknessUnit;
	}

	static bool AreEqual(Pen x, Pen y) {
		return x == y ||
			(x != null && y != null
			&& AreEqual(x.Brush, y.Brush)
			&& AreEqual(x.DashStyle, y.DashStyle)
			&& x.Thickness == y.Thickness);
	}

	static bool AreEqual(DashStyle x, DashStyle y) {
		return x == y ||
			(x != null && y != null
			&& x.Offset == y.Offset
			&& x.Dashes.SequenceEqual(y.Dashes));
	}
